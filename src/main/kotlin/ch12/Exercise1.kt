...
class StudentService(
    ...
    fun addStudent(addStudentRequest: AddStudentRequest) =
        addStudentRequest
            .let(studentFactory::produceStudent)
            ?.also(studentRepository::addStudent)

    fun getStudent(studentId: String) =
        studentId
            .let(studentRepository::getStudent)
            ?.also { logger.log("Student found: $it") }
            ?.let(studentFactory::produceExposed)

    fun getStudents(semester: String) =
        semester
            .let(::produceGetStudentsRequest)
            .let(studentRepository::getStudents)
            .also { logger.log("${it.size} students in $semester") }
            .map(studentFactory::produceExposed)

    private fun produceGetStudentsRequest(semester: String) =
        GetStudentsRequest()
            .apply {
